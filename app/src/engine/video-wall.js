 ...
 */
function drawAffineTriangle(ctx, source, sw, sh, s0, s1, s2, d0, d1, d2) {
...
/**
 * Draw a VideoFrame into an arbitrary convex quadrilateral using bilinear
 * mesh subdivision. Each sub-quad is rendered as two affine-mapped triangles.
 *
 * For mild perspective (e.g. a wall-mounted TV viewed off-axis) N=12 is
 * visually identical to true perspective projection.
 *
 ...
 */
function drawPerspectiveCell(ctx, source, quad, N = 12) {
  const [TL, TR, BR, BL] = quad;
  const sw = source.displayWidth  ?? source.codedWidth  ?? source.width;
  const sh = source.displayHeight ?? source.codedHeight ?? source.height;
