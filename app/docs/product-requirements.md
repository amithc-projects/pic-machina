...
| **TME** | Timeline Editor | Multi-track non-linear video editor. Supports magnetic snapping, drag-and-drop media pool, WebGL-powered stitcher transitions, keyframing, and compositing with live preview playback. |
| **SND** | Audio Studio | Standalone, native browser-based DAW (Digital Audio Workstation). Imports audio/video files and allows complex sound effects (Tone.js), cutting, clip joining, auto-splitting, and muting. Features automated parameter keyframing natively integrated with Web Audio scheduling. Exports final `.wav` mixes. Projects are saved locally as `.audioproject` packages using the native OS File System Access API. |
