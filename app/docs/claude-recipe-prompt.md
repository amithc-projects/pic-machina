...
Zumilabs Studio supports dynamic string interpolation using double curly braces: `{{variable_name}}`. By using analysis nodes early in the recipe, you can unlock sidecar metadata for later text nodes. Both simplified flat aliases and exact nested schema paths are supported at runtime:

*   **Location**: `{{sidecar.city}}` / `{{sidecar.country}}` (or nested `{{sidecar.geo.city}}` / `{{sidecar.geo.country}}`) — Extracted automatically from EXIF GPS data or geocoding.
*   **AI Analysis**: `{{sidecar.peopleLabel}}` / `{{sidecar.ocrText}}` (or nested `{{sidecar.analysis.computed.peopleLabel}}` / `{{sidecar.analysis.computed.ocrText}}`) — Populated by placing analysis nodes like **`ai-analyse-people`** or **`ai-smart-redact`** (with mode: 'extract') early in your recipe.
*   **Audio Subtitles**: `{{autoCaptions}}` — Variable generated by `ai-transcribe` containing SRT subtitle formatted text. Can be mapped directly into `overlay-subtitles` or `overlay-html-block`!
